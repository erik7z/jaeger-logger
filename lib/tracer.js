 ...
 */
var getDefaultTracer = function (serviceName, config) {
...
var Tracer = /** @class */ (function () {
    function Tracer(serviceName, optionsConfig) {
        this.serviceName = serviceName;
        this.config = (0, deepmerge_1.default)(exports.defaultConfig, optionsConfig);
        this.client = (0, jaeger_client_1.initTracer)(__assign({ serviceName: serviceName, sampler: {
                type: 'const',
                param: 1,
            } }, this.config), {
            logger: {
    ...
    Tracer.prototype.write = function (action, logData, context) {
        ...
        this.send(context, {
            ...
            details: __assign(__assign(__assign({}, (message ? { message: message } : {})), (data ? { data: data } : {})), (err
                ? {
                    isError: true,
                    err: __assign(__assign(__assign(__assign(__assign(__assign(__assign(__assign({}, (err.code ? { code: err.code } : {})), (err.status ? { status: err.status } : {})), (err.statusCode ? { statusCode: err.statusCode } : {})), (err.message ? { message: err.message } : typeof err === 'string' ? { message: err } : {})), (err.customMessage ? { customMessage: err.customMessage } : {})), (err.shortMessage ? { shortMessage: err.shortMessage } : {})), (err.detailedMessage ? { detailedMessage: err.detailedMessage } : {})), (err.stack ? { stack: err.stack } : {})),
                }
                : {})),
        });
    };
    Tracer.prototype.send = function (context, keyValuePairs, timestamp) {
