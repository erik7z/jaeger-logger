...
class Logger {
    ...
    static simplifyArgs(arguments_, excludeClasses = []) {
        ...
        else if (arguments_ instanceof Object || arguments_ instanceof Buffer || arguments_ instanceof node_stream_1.Stream) {
    ...
    static replaceStreamRecursive(argument, depth = 3) {
        if (argument instanceof node_stream_1.Stream)
            return 'Stream';
        if (depth > 0 && _.isObject(argument)) {
            _.forIn(argument, (value, key) => {
                // eslint-disable-next-line @typescript-eslint/ban-ts-comment
